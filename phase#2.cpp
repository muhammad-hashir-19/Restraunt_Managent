...
struct dish {
	...
	dish(string nam, float pri, string t, dish* temp) {
		srand(time(0));
		id = 9999 + (rand() % 100000);
		type = t;
		price = pri;
...
struct menu {
	...
	void add_dish(string name, string type, float price) {
		root = insertLevelOrder(root, name, price, type);
	}
	dish* insertLevelOrder(dish* root, string name, float price, string type) {
		if (!root) {
...
struct Customer {
	...
	Customer(string name, string email, string pass, string Location = NULL) {
		int I = 9999 + (rand() % 100000);
		Id = I;
		cut = I;
		Name = name;
...
struct Employees_hashmap {
	...
	void signup(string type) {
		if (type == "manager")
		{
			...
			cin >> type1;
			if (type1 == "simple") {
				cout << "Enter your User restaurant name : ";
				cin >> res;
			}
			else if (type1 == "Delivery_Driver")
			{
				cout << "Enter your User restaurant name : ";
				cin >> res;
			}
...
public:
	...
	void Enqueue(char val, int priority, int loca,string name)
	{
		Node* temp = new Node;
		temp->restraunt_name = name;
		temp->Order_name = val;
		temp->Front = priority;
		temp->location = loca;

		if (rear == NULL || priority < rear->Front)
		{
			temp->next = rear;
			rear = temp;
		}
		...
		{
			Node* Prior = rear;
			while (Prior->next != NULL && Prior->next->Front <= priority)
				Prior = Prior->next;
	...
	void Display(string* locations) //print queue 
	{
		Node* ptr = rear;
		if (rear == NULL)
			cout << "No order to Deliver\n";
		else
		{
			while (ptr != NULL)
			{
				cout << "Order-" << ptr->Order_name << "  Location: " << locations[ptr->location] << "    Status: Pending(Not Deliver yet)\n";
				ptr = ptr->next;
			}
		}
	}
	...
	int printSolution(int dist[], int location, string* AllLocations, int src)//Display the shortest Distance
	{

		for (int i = 0; i < V; i++)
			if (location == i) {
				cout << "After " << dist[i] << " minutes your Order is delivered  From " << AllLocations[src] << " to " << AllLocations[location] << endl;
				return dist[i];
			}
		return -1;
	}

	int ShortestPath(int graph[V][V], int src, int location, string* AllLocations)
	{
		int dist[V]; //distance from source to i

		bool sptSet[V]; //true if vertex i is included in shortest
		...
		return printSolution(dist, location, AllLocations, src);
	}
...
struct Restraunt_working {
	...
	void Add_restraunt(Employees_hashmap* e) {
		...
		cout << "Enter type of the restaurant : ";
		cin >> temp->Type;
		cout << "Enter Location of the restaurant : ";
		cin >> temp->Location;
		while (check_id(temp)) {
			srand(time(0));
	...
	// value , Non zero for onlinw
	//location = where to deliver
	//starting_location = starting point of the delivery driver
	int choose_restraunt(Customer* temp, int value = 0, int location = 0) {
		orders* o = new orders;
		string name;
		cout << "Enter name of the restaurant : ";
		cin >> name;
			...
			while (choice != 3) {
				...
				if (choice == 1) {
					...
					if (o->dishes != NULL) {
						...
						string  locations[] = { "F-5","F-6","F-7","F-8","F-9","F-10","F-11","F-12","G-5","G-6","G-7","G-8","G-9","G-10","G-11","G-12","G-13","G-14","H-8","H-9","H-10","H-11","I-8","I-9","I-10","I-11","I-12","I-14","I-15","I-16" };

						Weight weight[] = { {"F-12", "G-6", 12},{ "G-12", "H-9", 8 },{ "G-8", "F-12", 7 },{ "F-9", "I-8", 11 },{ "G-6", "F-9", 6 },{ "H-8", "I-10", 2 },{ "F-11", "H-9", 5 },{ "F-5", "F-8", 15 },{ "I-11", "I-9", 7 },{ "F-12", "F-6", 13 },{ "H-11", "G-7", 1 },{ "I-15", "G-12", 3 },{ "I-12", "H-8", 9 },{ "I-14", "G-6", 2 },{ "I-12", "I-9", 8 },{ "F-12", "I-16", 10 },{ "F-9", "F-12", 9 },{ "F-11", "G-9", 10 },{ "I-14", "G-11", 1 },{ "F-7", "G-9", 15 },{ "I-9", "H-10", 4 },{ "I-16", "G-14", 2 },{ "F-6", "I-15", 8 },{ "F-12", "I-14", 13 },{ "F-9", "H-11", 10 },{ "H-10", "I-10", 5 },{ "G-14", "G-12", 14 },{ "H-11", "G-13", 1 },{ "I-12", "I-15", 6 },{ "F-9", "G-10", 3 },{ "F-5", "F-10", 2 },{ "G-9", "G-14", 12 },{ "I-9", "F-11", 14 },{ "I-16", "F-7", 5 },{ "F-9", "I-11", 8 },{ "H-9", "G-7", 2 },{ "I-8", "G-11", 10 },{ "H-11", "I-12", 9 },{ "G-12", "F-8", 4 },{ "F-10", "F-7", 6 },{ "F-5", "I-16", 7 },{ "H-8", "I-16", 13 },{ "H-9", "G-8", 6 },{ "I-11", "F-8", 4 },{ "I-12", "G-12", 15 },{ "F-6", "I-9", 3 },{ "I-14", "I-12", 8 },{ "F-11", "G-5", 1 },{ "G-8", "I-9", 5 },{ "H-8", "G-5", 11 },{ "F-10", "F-11", 6 },{ "G-10", "F-12", 5 },{ "G-9", "H-11", 7 },{ "H-9", "F-9", 11 },{ "F-12", "F-5", 2 },{ "F-6", "I-12", 9 },{ "I-15", "F-12", 14 },{ "G-5", "I-14", 12 },{ "F-9", "G-6", 15 },{ "I-15", "F-9", 7 },{ "G-8", "G-7", 2 },{ "F-12", "F-6", 14 },{ "H-9", "I-8", 3 },{ "G-10", "F-11", 1 },{ "I-8", "I-15", 2 },{ "F-9", "I-16", 9 },{ "H-9", "I-10", 3 },{ "H-8", "G-11", 6 },{ "H-11", "G-12", 4 },{ "I-9", "G-12", 7 },{ "I-8", "G-8", 10 },{ "H-10", "I-8", 5 },{ "G-12", "I-11", 12 },{ "I-10", "F-5", 15 },{ "F-10", "G-10", 13 },{ "F-6", "F-12", 1 },{"I-14", "H-11", 2 },{ "G-11", "F-10", 5 },{ "I-10", "I-16", 14 },{ "F-10", "F-6", 3 },{ "F-12", "F-8", 6 },{ "F-8", "F-6", 7 },{ "G-9", "F-11", 11 },{ "F-6", "H-9", 12 },{ "F-8", "G-9", 4 },{ "F-12", "H-11", 3 },{ "H-11", "I-15", 5 },{ "I-12", "G-14", 8 },{ "G-6", "F-9", 10 },{ "I-9", "G-11", 9 } };

						int graph[30][30] = { 0 };
						for (int i = 0; i < sizeof(locations) / sizeof(locations[0]); i++)
						{
							for (int k = 0; k < sizeof(locations) / sizeof(locations[0]); k++)
							{
								for (int j = 0; j < 90; j++)
								{
									if ((locations[i] == weight[j].loc1 && locations[k] == weight[j].loc2) || (locations[i] == weight[j].loc2 && locations[k] == weight[j].loc1))
									{
										graph[i][k] = weight[j].dist;
									}
								}
							}
						}
						...
						if (name == "res1")
						{
							path = drivers->ShortestPath(graph, 0, location, locations);
						}
						else if (name == "res2")
						{
							path = drivers->ShortestPath(graph, 1, location, locations);
						}
	...
	void execute_orders(string name, Employees* e, Delivery_drivers* d1,int order_name,int Path,int location) {
		Restaurants_node* hold = search_restraunt(name);
		...
		if (ord == NULL) {
			...
			do {
				if (ord->Type == p) {
					...
					d1->Enqueue((char)order_name, Path, location,hold->Name);//to_string convert int to string
					order_name++;
...
int main() {
	...
	Delivery_drivers* d1=new Delivery_drivers;
	int location;
	int Path = -1;
	int order_name = 65;
	int prev_location[10] = { 0 };
	int Totallocations = 0;
	int k = 0;
	string DeliveryAreas[] = { "F-5","F-6","F-7","F-8","F-9","F-10","F-11","F-12","G-5","G-6","G-7","G-8","G-9","G-10","G-11","G-12","G-13","G-14","H-8","H-9","H-10","H-11","I-8","I-9","I-10","I-11","I-12","I-14","I-15","I-16" };
	...
	cout << "   Name      Password         Location \n";
	cout << "1. cust1   12345678Aa@           F-7" << endl;
	cout << "2. cust2   12345678Bb@			  F-8" << endl;
	cout << "\nTwo employee managers are added into the system with following name,type and password" << endl;
	cout << "   Name    Type				  Password      restaurant\n";
	...
	cout << "4. emp4   Delivery_Driver      12345678Ee@        res1" << endl;
	...
	Customer* c1 = new Customer("cust1", "cust1@gmail.com", "12345678Aa@", "F-7");
	temp->insert(c1);
	Customer* c2 = new Customer("cust2", "cust2@gmail.com", "12345678Bb@", "F-8");
	temp->insert(c2);
	Customer* c3 = new Customer("cust3", "cust3@gmail.com", "12345678Cc@", "F-6");
	temp->insert(c3);
	...
	temp1->insert(e4);
	Employees* e5 = new Employees("emp5", "emp5@gmail.com", "12345678Ee@", "Delivery_Driver", "res1");
	temp1->insert(e5);
	...
	res->search_restraunt("res1")->menue->add_dish("Andashami", "FastFood", 30);
	...
	res->search_restraunt("res2")->menue->add_dish("Pizza", "FastFood", 30);
	res->search_restraunt("res2")->menue->levelOrderTraversal();
	cout << "<-------------------------Reviews-------------------------------->\n";
	cout << "  customer      restaurant    description                             ratings\n1.cust1	        res1	     This restraunt has good desi food       5\n2.cust2	        res2	     This restraunt has good western food    4\n\n";
	heap_reviews* h = new heap_reviews;
	h->add_reviwe(res->search_restraunt("res1"), "cust1", "This restaurant has good desi food", 5);
	h->add_reviwe(res->search_restraunt("res2"), "cust1", "This restaurant has good western food", 4);
	int choice1 = 5;
	while (choice1 != 0) {
		...
		case 1:
		{
			...
			while (option != 0) {
				...
				case 2: {
					int order_choice;
					cout << "\nPress1: If you want to order online. ";
					cin >> order_choice;
					if (order_choice == 1)
					{
						cout << "\nOnline service is available only these areas:\n";
						for (int i = 0; i < (sizeof(DeliveryAreas) / sizeof(DeliveryAreas[0])); i++)
						{
							cout << i << ". " << DeliveryAreas[i] << endl;
						}
						...
						for (int i = 0; i < (sizeof(DeliveryAreas) / sizeof(DeliveryAreas[0])); i++)
						{
							if (DeliveryAreas[i] == DeliveryAreas[location])
							{
								check = true;
							}
						...
						if (check == true)
						{
							Customer* Hold = temp->search(temp->n, temp->p);
							Hold->Location = location;
							temp->n = location;
						}
						else
					...
					Customer* c = temp->search(temp->n, temp->p);
					if (check == true)
						Path = res->choose_restraunt(c, 1, location);
					else
						Path = res->choose_restraunt(c, 0, location);
		...
		case 2:
		{
			...
			if (boo1 == false) {
				...
				while (option != 0) {
					...
					case 5: {
						if (c->restraunt_Name != "") {
							res->execute_orders(c->restraunt_Name, c,d1, order_name,Path,location);
						}
						else {
							cout << "You have no restraunt!!\n";
						}
					...
					case 9: {
						...
						if (c->Type == "Delivery_Driver")
						{
							...
							else
							{
								for (int i = 0; i < sizeof(locations) / sizeof(locations[0]); i++)
								{
									if (locations[i] == locations[d1->rear->location])
									{
										prev_location[++Totallocations] = i;
									}
								}
								cout << "Following Orders are pending...\n";
								d1->Display(DeliveryAreas);

								cout << "\nPress1: To deliver the order" << endl;
								cin >> ch;
								if (ch == 1)
								{
									path = d1->ShortestPath(graph, prev_location[k++], d1->rear->location, locations);
									cout << d1->rear->Order_name << " is Delivered" << endl;
									d1->Dequeue();

									for (int i = 0; i < k; i++)
									{
										int temp = prev_location[i + 1];
										prev_location[i] = temp;
										prev_location[i + 1] = 0;
									}
								}
							}
							...
							cout << "You are not an delivery driver!!" << endl;
						}
					}
					default:
						break;
